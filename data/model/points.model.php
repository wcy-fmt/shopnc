...
class pointsModel {
	 ...
	 * @param  string $stage 操作阶段 regist(注册),login(登录),comments(评论),order(下单),system(系统),other(其他),pointorder(购物点礼品兑换),app(同步购物点兑换)
	 * @param  array $insertarr 该数组可能包含信息 array('pl_memberid'=>'会员编号','pl_membername'=>'会员名称','pl_adminid'=>'管理员编号','pl_adminname'=>'管理员名称','pl_points'=>'购物点','pl_desc'=>'描述','orderprice'=>'订单金额','order_sn'=>'订单编号','order_id'=>'订单序号','point_ordersn'=>'购物点兑换订单编号');
		...
		switch ($stage){
			...
			case 'order':
				...
				if ($insertarr['orderprice']){
					...
					// todo:wlpro 如果获得的购物点大于设定的购物点上限，使用上限购物点
//                    如果购物点上限为-1，表示不设限
                    echo intval(C('points_ordermax'));
					if ($insertarr['pl_points'] > intval(C('points_ordermax')) && intval(C('points_ordermax'))>=0){
            ...
            case 'pointorder':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '兑换礼品信息'.$insertarr['point_ordersn'].'消耗购物点';
                }
                break;
            case 'pointorder_ext':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '购物点换购'.$insertarr['point_ordersn'].'消耗购物点';
                }
                break;
                case 'return_points':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '购物点换购'.$insertarr['point_ordersn'].'返还购物点';
                }
                break;
            case 'app':
				if (!$insertarr['pl_desc']){
		...
		$value_array['pl_ordersn'] = $insertarr['point_ordersn'];
		$result = false;
		...
		if ($result){
			...
			switch ($stage){
                case 'return_points':
                    $upmember_array['member_points_ext'] = array('exp', 'member_points_ext+' . $insertarr['pl_points']);
                    break;
                case 'pointorder_ext':
                    $member_info=$insertarr['member_info'];
                    // 优先扣购物点
                    if($member_info['member_points']>=10000){
                        $upmember_array['member_points'] = array('exp', 'member_points+' . $insertarr['pl_points']);
                    }else if ($member_info['member_points_ext']>=10000){//次要扣返点
                        $upmember_array['member_points_ext'] = array('exp', 'member_points_ext+' . $insertarr['pl_points']);
                    }
                    break;
                default:

...
class pointcartModel extends Model {
	...
	/**
	 * 获得符合条件的购物车商品列表同时计算运费、总购物点数等信息，用于下单过程
	 */
	...
	public function checkPointEnough($points, $member_id, $member_info = array()){
	    ...
	    if (!$member_info){
	        $member_info = Model('member')->getMemberInfoByID($member_id, 'member_points,member_points_ext');
	    }
	    if (intval($member_info['member_points']) < $points){
	        return array('state'=>false,'msg'=>'购物点不足，暂时不能兑换');
	    }
	    return array('state'=>true);
	}
}
...
class buyLogic {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id,$use_points=0) {

        //得到购买商品信息
        if ($ifcart) {
            $result = $this->getCartList($cart_id, $member_id);
        } else {
            $result = $this->getGoodsList($cart_id, $member_id, $store_id,$use_points);
        }
     ...
     */
    public function getGoodsList($cart_id, $member_id, $store_id,$use_points=0) {
        ...
        //todo:wlpro 如果是购物点换购，变价
        if($use_points==1) {
            $goods_info['goods_price'] *= 0.10;
            $goods_info['goods_price_discount'] *= 0.90;
        }

        $model_goods = Model('goods');
        $goodscommon_info = $model_goods->getGoodeCommonInfoByID($goods_info['goods_commonid']);

        //todo :计算返佣金额
        $goods_info['goods_commis'] =$goods_info['goods_price']-$goodscommon_info['goods_costprice'];
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        list($store_cart_list,$store_goods_total,$store_goods_commis_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
        $result['store_cart_list'] = $store_cart_list;
        $result['store_goods_total'] = $store_goods_total;
        $result['store_goods_commis_total'] = $store_goods_commis_total;
     ...
     * @return array
     */
    public function buyStep2($post, $member_id, $member_name, $member_email,$use_points=0) {
        ...
        try {
            ...
            //第2步 得到购买商品信息
            $this->_createOrderStep2($use_points);
    
            //第3步 得到购买相关金额计算等信息
            $this->_createOrderStep3($use_points);
    
            //第4步 生成订单
            $this->_createOrderStep4($use_points);
     ...
     *
     */
    private function _createOrderStep2($use_points=0) {
        $post = $this->_post_data;
    ...
    private function _createOrderStep3() {
        ...
        $this->_order_data['store_goods_commis_total'] = $store_goods_commis_total;
     ...
     */
    private function _createOrderStep4($use_points=0) {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            $order['order_amount'] = $store_final_order_total[$store_id];
            //todo:wlpro返佣保存
            $order['order_commis_amount'] = $store_goods_commis_total[$store_id];

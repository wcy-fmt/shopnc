...
class buy_1Logic {
    ...
    public function calcCartList($store_cart_list) {
        ...
        //todo:存放每个店铺的商品返佣总金额
        $store_goods_commis_total = array();
        //存放本次下单所有店铺商品总金额
        $order_goods_total = 0;
    
        foreach ($store_cart_list as $store_id => $store_cart) {
            $tmp_amount = 0;
            $tmp_commis=0;//todo:平台返佣
            foreach ($store_cart as $key => $cart_info) {
                $store_cart[$key]['goods_total'] = ncPriceFormat($cart_info['goods_price']  * $cart_info['goods_num']);
                $store_cart[$key]['goods_discount_total'] =  $cart_info['goods_price_discount']  * $cart_info['goods_num'];
                $store_cart[$key]['goods_commis_total'] = ncPriceFormat($cart_info['goods_commis']  * $cart_info['goods_num']);
                $store_cart[$key]['goods_image_url'] = cthumb($store_cart[$key]['goods_image']);
                $tmp_amount += $store_cart[$key]['goods_total'];
                $tmp_commis += $store_cart[$key]['goods_commis_total'];
            }
            $store_cart_list[$store_id] = $store_cart;
            $store_goods_total[$store_id] = ncPriceFormat($tmp_amount);
            $store_goods_commis_total[$store_id] = ncPriceFormat($tmp_commis);
        }
        return array($store_cart_list,$store_goods_total,$store_goods_commis_total);
    }
...
//         if (empty($pd_payment_info)) return;
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        //todo :计算返佣金额
        $new_array['goods_commis'] =$goods_info['goods_price']-$goodscommon_info['goods_costprice'];
        $new_array['goodscommon_info'] =$goodscommon_info;

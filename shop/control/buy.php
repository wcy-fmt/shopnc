...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        //得到购买数据
        $logic_buy = Logic('buy');
        $result = $logic_buy->buyStep1($_POST['cart_id'], $_POST['ifcart'], $_SESSION['member_id'], $_SESSION['store_id'], $_POST['use_points']);
        if(!$result['state']) {
        ...
        Tpl::output('use_points', $_POST['use_points']);
        Tpl::output('store_goods_total', $result['store_goods_total']);
        Tpl::output('store_goods_commis_total', $result['store_goods_commis_total']);
    ...
    public function buy_step2Op() {
        ...
        if ($_SESSION['member_id'] <= 0){
            showMessage('会员信息错误', 'index.php?act=cart', 'html', 'error');
        }
        //todo:wlpro 如果使用积分换购，检查积分
        if($_POST['use_points']) {
            $member_info = Model('member')->getMemberInfoByID($_SESSION['member_id'], 'member_points,member_points_ext');

            if (intval($member_info['member_points']) < 10000) {
                if (intval($member_info['member_points_ext']) < 10000) {
                    showMessage('购物点不足，暂时不能兑换', 'index.php?act=cart', 'html', 'error');
                }
            }
        }
        ...
        $result = $logic_buy->buyStep2($_POST, $_SESSION['member_id'], $_SESSION['member_name'], $_SESSION['member_email'],$_POST['use_points']);
        if(!$result['state']) {
            showMessage($result['msg'], 'index.php?act=cart', 'html', 'error');
        }

        //转向到商城支付页面
        redirect('index.php?act=buy&op=pay&pay_sn='.$result['data']['pay_sn'].'&use_points='.$_POST['use_points']);
    }
    ...
    public function payOp() {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if($order_info['use_points']==1) {
                $member_info = Model('member')->getMemberInfoByID($_SESSION['member_id']);
                // TODO：wlpro检查剩余购物点
                if ($member_info['member_points'] < 10000 && $member_info['member_points_ext'] < 10000) {
                    showMessage(' 购物点低于10000点，暂不能换购和支付', 'index.php?act=member_order', 'html', 'error');
                }
            }
        ...
        Tpl::output('use_points','use_points');
	...
	public function pay_okOp()
    {
        ...
        // TODO:升级会员等级
        // 不是购物点换购的订单，进行会员等级升降级
        if ($use_points == 0) {
            $exppoints_model = Model('exppoints');
            $exppoints_model->saveExppointsLog('boss', array('exp_memberid' => $member_info['member_id'], 'exp_membername' => $member_info['member_name'],'member_exppoints' => $member_info['member_exppoints'],'amount'=>$pay_amount,'commis_amount'=>$pay_commis_amount,'pay_sn'=>$pay_sn), true);

      ...
      <!-- S 购买按钮 -->
        <div class="ncs-btn"><!-- S 提示已选规格及库存不足无法购买 -->
          ...
          <?php }?>
              ...
              <!-- 购物点购买-->
              <?php echo '购物点:'.$output['member_info']['member_points'].'&nbsp;&nbsp;扩展购物点:'.$output['member_info']['member_points_ext']; ?><br/>

              <a href="javascript:void(0);" nctype="buynow_points_submit" class="buynow <?php if ($output['goods']['goods_state'] == 0 || ($output['member_info']['member_points'] < 10000 && ($output['member_info']['member_points_ext'] < 10000))) {?>no-buynow<?php }?>" title="购物点换购，当前购物点不足">购物点换购</a>
...
<form id="buynow_form" method="post" action="<?php echo SHOP_SITE_URL;?>/index.php">
  ...
  <input id="use_points" name="use_points" type="hidden"/>
</form>
...
$(function(){
    ...
    $('a[nctype="addcart_submit"]').click(function(){
        ...
        <?php if($output['member_info']['level_points']>=0) {?>
        // 购物点购买
        $('a[nctype="buynow_points_submit"]').click(function(){
            buynow(<?php echo $output['goods']['goods_id']?>,checkQuantity(),1);
        });
        <?php }?>
...
// 立即购买js
function buynow(goods_id,quantity,use_points){
    if(_.isUndefined(use_points)  ){// 是否购物点购买
        use_points=0;
    }
<?php if ($_SESSION['is_login'] !== '1'){?>
	login_dialog();
